...
def import_isos_from_yaml(path: str, default_fmv: Decimal) -> typing.List[IncentiveStockOption]:
...
def import_rsus_from_yaml(path: str, default_fmv: Decimal) -> typing.List[RestrictedStockUnit]:
    ...
    for equity in equity_dict:
        equity_class = equity.pop("class")
        if equity_class == "RSU":
            if "vest_fair_market_value" not in equity:
                equity["vest_fair_market_value"] = default_fmv
            rsus.append(
                dacite.from_dict(
                    RestrictedStockUnit,
                    equity,
